...
export interface SwipePendingResponse {
  people: SwipePerson[];
}

export const fetchSwipePending = (skipSessionId: string, limit = 100) =>
  jsonFetch<SwipePendingResponse>(`/api/verification/swipe-pending?skipSessionId=${encodeURIComponent(skipSessionId)}&limit=${limit}`);

export interface VerificationTask {
  id: string;
  kind: "face_name" | "same_person";
  status: string;
...
export const fetchVerificationTasks = (params: { status?: string; kind?: string; limit?: number; skipSessionId?: string } = {}) => {
  const search = new URLSearchParams();
  if (params.status) search.set("status", params.status);
  if (params.kind) search.set("kind", params.kind);
  if (params.limit) search.set("limit", String(params.limit));
  if (params.skipSessionId) search.set("skipSessionId", params.skipSessionId);
  return jsonFetch<{ tasks: VerificationTask[] }>(`/api/verification/tasks?${search.toString()}`);
};

export const decideVerificationTask = (taskId: string, decision: "yes" | "no" | "skip", correctPersonId?: string) =>
  jsonFetch<{ ok: boolean }>(`/api/verification/tasks/${taskId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ decision, correctPersonId }),
  });

export const generateVerificationTasks = (faceNameLimit = 20, samePersonLimit = 20) =>
  jsonFetch<{ ok: boolean }>("/api/verification/generate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ faceNameLimit, samePersonLimit }),
  });

export interface BulkGroup {
  groupId: string;
  bestMatch: { personId: string; personName: string; distance: number } | null;
  faces: Array<{
    faceId: string;
    boundingBox: { x: number; y: number; width: number; height: number };
    photoId: string;
...
export interface BulkPendingResponse {
  groups: BulkGroup[];
}

export const fetchBulkPending = (limit = 50) =>
  jsonFetch<BulkPendingResponse>(`/api/verification/bulk-pending?limit=${limit}`);
...
export const bulkDecide = (decisions: BulkDecideRequest[]) =>
  jsonFetch<{ ok: boolean; updated: number }>("/api/verification/bulk-decide", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
